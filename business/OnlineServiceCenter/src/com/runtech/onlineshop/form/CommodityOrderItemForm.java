...
public class CommodityOrderItemForm extends CommodityOrderItem implements
	...
	public void calcPrice() {
		...
		BigDecimal priceQuantity = this.getCommodity().getPriceQuantity();
		if(priceQuantity==null){
			priceQuantity = new BigDecimal(1);
		}
		if(this.getCommodityPromotion()!=null){
			price = this.getCommodityPromotion().getPrice().multiply(priceQuantity).multiply(new BigDecimal(this.getCommodityNumber()));
		}else{
			price = this.getCommodity().getPrice().multiply(priceQuantity).multiply(new BigDecimal(this.getCommodityNumber()));
		}
		...
		BigDecimal realPrice = this.getPrice().multiply(discount);
		realPrice = realPrice.setScale(2,BigDecimal.ROUND_HALF_UP);
		this.setRealPrice(realPrice);
		Integer point = this.getCommodity().getPoint()*this.getCommodityNumber();
		this.setPoint(point);
	}
	...
	public String getDiscountPercent(){
		...
		return df.format(this.getDiscount());
	}
