...
<s:form>
	...
	<tt:table value="specialModelList" name="modelElement">
		...
		<tt:column title="%{getText('text.content.siteModule')}">
...
<s:form enctype="multipart/form-data">
	...
	<s:select name="siteModuleId" label="%{getText('text.commodity.siteModuleId')}" list='@com.runtech.onlineshop.form.SiteModuleForm@getAllModules(@com.runtech.web.util.Constant@CODE_SITE_MODULE_ROOT," - ")' listKey="id" listValue="name" emptyOption="true"/>
	<s:if test="%{positionCode==null}">
		<s:select name="positionCode" label="%{getText('text.content.position')}" list="@com.runtech.onlineshop.form.SystemCodeForm@getCodes('articlePosition')" listKey="code" listValue="name"/>
	</s:if>
	<s:else>
		<s:hidden name="positionCode" value="%{positionCode}"/>
	</s:else>
