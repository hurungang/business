...
<s:form name="ajaxForm" action="ajax/customerContract" target="_blank" >
	<div class="table_container">
	<runtech:page pager="${specialPager}" action="../ajax/customerContract.do" command="paging"></runtech:page>
	<tt:table value="searchResult" name="modelElement"  cssClass="listTable">
		...
		<tt:column title="%{getText('text.contract.id')}">
			<a rel="modal" href="<%=request.getContextPath()%>/protect/ajax/<s:property value='pageCode'/>.do?userId=<s:property value='userId'/>&formId=<s:property value='id'/>">
				<s:property value="id" />
			</a>
		</tt:column>
	...
	<div class="button_container">
		<input type="button" onclick="switchTab('<s:property value="pageCode"/>','form_tab');" value="<s:property value="%{getText('text.submit.add')}"/>" class="button"/>
	</div>
</s:form>

					</div>
					<div class="tab-content" id="<s:property value="pageCode"/>_form_tab">
<s:form name="ajaxForm" cssClass="ajaxForm" enctype="multipart/form-data" action="ajax/customerContract" >
